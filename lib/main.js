...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield exec.exec(`git archive --format=tar.gz -o "${name}-${version}.tar.gz" --prefix="${name}-${version}/" ${ref}`);
            ...
            try {
                yield exec.exec(`rpmbuild -ba --define "version ${version}" /github/home/rpmbuild/SPECS/${specFile}`);
            }
            catch (err) {
            ...
            let rpmOutput = '';
            yield cp.exec('ls /github/home/rpmbuild/RPMS/x86_64', (err, stdout, stderr) => {
                if (err) {
                    //some err occurred
                    console.error(err);
                }
